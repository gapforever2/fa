...
UAL0301 = ClassUnit(CommandUnit) {
    Weapons = {
        RightReactonCannon = ClassWeapon(ADFReactonCannon) {
            OnCreate = function(self)
                ADFReactonCannon.OnCreate(self)
    ...
    CreateEnhancement = function(self, enh)
        ...
        elseif enh == 'StabilitySuppressantRemove' then
            ...
            wep:AddDamageRadiusMod(bp.NewDamageRadiusMod or 0)
            wep:ChangeMaxRadius(bp.MaxRadius or 0)
            wep:ChangeRateOfFire(self:GetBlueprint().Weapon[1].RateOfFire or 1)
            --oblivion gun
        elseif enh == 'OblivionWeapon' then
            local wep = self:GetWeaponByLabel('FrontTurret')
            wep:ChangeMaxRadius(bp.NewMaxRadius or 40)
        elseif enh == 'OblivionWeaponRemove' then
            local wep = self:GetWeaponByLabel('FrontTurret')
            wep:ChangeMaxRadius(bp.MaxRadius or 0)
        end
    end,

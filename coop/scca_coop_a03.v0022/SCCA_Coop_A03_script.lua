-- ****************************************************************************
-- **
-- **  File     :  /maps/scca_coop_a03.v0022/SCCA_Coop_A03_script.lua
...
local M2UEFAI = import('/maps/scca_coop_a03.v0022/SCCA_Coop_A03_m2uefai.lua')
local M3UEFAI = import('/maps/scca_coop_a03.v0022/SCCA_Coop_A03_m3uefai.lua')
local Objectives = import('/lua/SimObjectives.lua')
local OpStrings = import('/maps/scca_coop_a03.v0022/SCCA_Coop_A03_strings.lua')
...
function OnPopulate(scenario)
    ...
    ScenarioInfo.ErisCDR = ScenarioFramework.SpawnCommander('Eris', 'Eris', false, LOC '{i CDR_Eris}', false, false,
        {'ShieldAeon', 'HeatSinkAeon', 'GAP_CrysalisBeamAdvancedAeon', 'ResourceAllocationAeon'})
	...
	ScenarioInfo.ErisAirPatrol = ScenarioUtils.CreateArmyGroupAsPlatoon('Eris', 'AirUnits', 'AttackFormation')
    for k, v in ScenarioInfo.ErisAirPatrol:GetPlatoonUnits() do
        v:AdjustHealth(v, Random(1, v:GetHealth()/3) * -Difficulty)
    end
    ScenarioFramework.PlatoonPatrolChain(ScenarioInfo.ErisAirPatrol, 'Air_Chain')
...
function OnStart(self)
    ...
    ScenarioFramework.RestrictEnhancements({
        'ResourceAllocationAeon',
        'ResourceAllocationAdvancedAeon',
        'T3EngineeringAeon',
        'ShieldHeavyAeon',
        'TeleporterAeon'
    })
...
function StartMission1()
	...
	platoon = ArmyBrains[Player1]:MakePlatoon('', '')
    for _, v in ScenarioInfo.ErisAirPatrol:GetPlatoonUnits() do
        if not v:IsDead() then
            local unit = ScenarioFramework.GiveUnitToArmy(v, Player1)
            ArmyBrains[Player1]:AssignUnitsToPlatoon(platoon, {unit}, 'attack', 'AttackFormation')
        end
    end
    ScenarioFramework.PlatoonPatrolChain(platoon, 'Air_Chain')
...
function IntroMission2()
    ...
    ScenarioInfo.ArnoldCDR = ScenarioFramework.SpawnCommander('UEF', 'Arnold', false, LOC '{i CDR_Arnold}', false, false,
        {'DamageStabilizationUEF', 'ShieldUEF', 'HeavyAntiMatterCannonUEF'})
